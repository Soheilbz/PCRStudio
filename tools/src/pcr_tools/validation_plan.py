...
PLANS: dict[str, dict[str, Any]] = {
    ...
    "colony-pcr": {
        ...
        "not_computed": ["colony/sample input optimum, lysis quality, host inhibition, empirical cycling suitability and construct identity"],
    },
    "qpcr-sybr": {
        "items": [
            *COMMON,
    ...
    "nested-pcr": {
        "items": [
            *COMMON,
            _item("round_one_control", "Outer-round amplification control", kind="control", phase="bench-run", source="nested PCR validation practice", why="The inner reaction cannot rescue an unobserved or contaminated outer round without changing the interpretation of the assay."),
            _item("round_two_control", "Inner-round amplification control", kind="control", phase="bench-run", source="nested PCR validation practice", why="The nested product must be attributable to the intended inner pair and transfer scheme, not carry-over or an unrelated outer product."),
            _item("carryover_control", "Carry-over contamination control", kind="control", phase="bench-run", source="nested PCR validation practice", why="Opening or transferring the first-round product materially increases contamination risk and cannot be validated in silico."),
            _item("nested_product_identity", "Nested product identity", kind="measurement", phase="post-run", source="nested PCR validation practice", why="Containment geometry predicts an inner product but does not prove the observed nested product identity."),
        ],
        "not_computed": ["round-to-round carry-over, empirical enrichment, contamination rate or observed product identity"],
    },
    "inverse-pcr": {
        "items": [
            *COMMON,
            _item("template_topology", "Circularized/digested template topology", kind="measurement", phase="pre-run", source="inverse PCR validation practice", why="Outward-primer geometry is only meaningful if the physical digest/circularization state matches the selected branch."),
            _item("junction_identity", "Circularization junction identity", kind="measurement", phase="post-run", source="inverse PCR validation practice", why="The reconstructed junction is a sequence model, not evidence that the bench preparation created that junction."),
            _item("linear_template_control", "Linear-template control", kind="control", phase="bench-run", source="inverse PCR validation practice", why="A control distinguishes the intended circular-template mechanism from amplification that can occur on residual linear material."),
        ],
        "not_computed": ["digest completeness, ligation/circularization efficiency, methylation-dependent cleavage or physical template topology"],
    },
    "qpcr-probe": {
        "items": [
            *COMMON,
            _item("raw_fluorescence", "Raw fluorescence traces", kind="measurement", phase="post-run", source="MIQE 2.0", why="Raw traces are required to review baseline, Cq determination and fluorescence behavior; sequence design cannot create them."),
            _item("cq_method", "Cq method and threshold", kind="measurement", phase="analysis", source="MIQE 2.0", why="Cq is analysis-method dependent and cannot be inferred from the oligo sequences."),
            _item("standard_curve", "Standard curve, efficiency and quantitative range", kind="measurement", phase="assay-validation", source="MIQE 2.0", why="Efficiency, linearity, LOD and LLOQ are empirical assay properties that require dilution-series evidence."),
            _item("probe_channel_configuration", "Probe dye/channel and optical configuration", kind="decision", phase="pre-run", source="qPCR platform validation practice", why="A geometrically valid probe is not evidence that the reporter/quencher and instrument channel configuration are compatible."),
            _item("probe_signal_specificity", "Probe-signal specificity", kind="measurement", phase="assay-validation", source="MIQE 2.0", why="Observed probe signal, non-template behavior and target discrimination must be demonstrated on the selected chemistry/platform."),
        ],
        "not_computed": ["Cq, amplification efficiency, fluorescence amplitude, LOD/LLOQ, optical cross-talk or observed probe cleavage"],
    },
    "arms-pcr": {
        "items": [
            *COMMON,
            _item("known_genotype_controls", "Known-genotype allele controls", kind="control", phase="assay-validation", source="allele-specific PCR validation practice", why="Allele discrimination is an empirical extension property and must be demonstrated against known genotypes."),
            _item("heterozygote_control", "Heterozygous control, when biologically applicable", kind="control", phase="assay-validation", source="allele-specific PCR validation practice", why="A heterozygous sample tests simultaneous allele detection and exposes preferential amplification that sequence scoring cannot certify.", required=False),
            _item("genotype_concordance", "Genotype concordance", kind="measurement", phase="assay-validation", source="genotyping validation practice", why="Predicted 3′ discrimination does not establish analytical genotype calling accuracy."),
        ],
        "not_computed": ["allelic amplification ratio, false-call rate, genotype concordance or matrix-specific discrimination"],
    },
    "tetra-primer-arms": {
        "items": [
            *COMMON,
            _item("known_genotype_controls", "Known-genotype controls", kind="control", phase="assay-validation", source="tetra-ARMS validation practice", why="All genotype band patterns must be observed with known controls rather than inferred from product geometry alone."),
            _item("diagnostic_band_resolution", "Diagnostic band resolution", kind="measurement", phase="post-run", source="tetra-ARMS validation practice", why="Predicted product-size separation does not guarantee resolvable bands in the actual gel/capillary system."),
            _item("four_primer_balance", "Four-primer reaction balance", kind="measurement", phase="assay-validation", source="tetra-ARMS validation practice", why="Competition among four primers is empirical and can change genotype signal balance despite acceptable pairwise thermodynamics."),
        ],
        "not_computed": ["observed genotype pattern, allele balance, gel resolution or genotype concordance"],
    },
    "kasp": {
        "items": [
            _item("no_template_control", "No-template control", kind="control", phase="bench-run", source="KASP genotyping validation practice", why="Defines the no-template cluster/background for endpoint genotype calling."),
            _item("known_genotype_controls", "Known-genotype controls", kind="control", phase="assay-validation", source="KASP genotyping validation practice", why="FAM/HEX cluster identity must be anchored to known genotypes, not inferred from tail assignment alone."),
            _item("instrument_optics", "Instrument dye/ROX configuration", kind="decision", phase="pre-run", source="KASP platform guidance", why="Endpoint FRET readout depends on instrument optical settings and reference-dye policy."),
            _item("cluster_review", "Endpoint cluster quality and call review", kind="measurement", phase="post-run", source="KASP genotyping validation practice", why="Cluster separation, call rate and ambiguous wells are observed data properties and cannot be predicted from sequence design."),
            _item("genotype_concordance", "Genotype concordance", kind="measurement", phase="assay-validation", source="genotyping validation practice", why="Analytical genotype-call accuracy requires comparison with known or orthogonally confirmed samples."),
        ],
        "not_computed": ["endpoint fluorescence clusters, call rate, cluster separation, ROX behavior or genotype concordance"],
    },
    "lamp": {
        "items": [
            _item("no_template_control", "No-template control", kind="control", phase="bench-run", source="LAMP validation practice", why="LAMP is highly productive and contamination/background signal must be measured in the actual readout."),
            _item("positive_control", "Positive control", kind="control", phase="bench-run", source="LAMP validation practice", why="Confirms the selected Bst-family chemistry and readout can detect the intended target."),
            _item("readout_validation", "Readout-specific validation", kind="measurement", phase="assay-validation", source="LAMP assay validation practice", why="Fluorescence, colorimetric and turbidity readouts have different decision behavior; the chosen readout must be validated empirically."),
            _item("product_identity", "LAMP product identity/orthogonal confirmation", kind="measurement", phase="post-run", source="LAMP validation practice", why="Complex LAMP products and nonspecific amplification cannot be certified from primer geometry alone."),
            _item("contamination_control", "Carry-over/amplicon contamination control", kind="control", phase="bench-run", source="LAMP validation practice", why="High product yield makes contamination control a physical workflow requirement, not a sequence-derived property."),
        ],
        "not_computed": ["time-to-positive, color threshold, fluorescence threshold, matrix inhibition, amplification yield or diagnostic sensitivity/specificity"],
    },
    "universal-primers": {
        "items": [
            _item("alignment_provenance", "Alignment/accession provenance", kind="decision", phase="design-review", source="PCRStudio universal-primer contract", why="Coverage claims are only meaningful for a versioned, curated input panel and alignment authority.", computable=True),
            _item("coverage_strata", "Target coverage by relevant strata", kind="measurement", phase="assay-validation", source="universal-primer validation practice", why="Overall in-silico coverage can hide lineage/strain/taxon-specific failures and requires empirical inclusivity review."),
            _item("degenerate_pool_inclusivity", "Degenerate-pool empirical inclusivity", kind="measurement", phase="assay-validation", source="universal-primer validation practice", why="Concrete-member coverage does not measure synthesis bias or competitive amplification within the physical degenerate pool."),
            _item("non_target_panel", "Non-target/exclusion panel", kind="measurement", phase="assay-validation", source="universal-primer validation practice", why="Broad target coverage must be balanced against empirical non-target amplification in the intended application."),
        ],
        "not_computed": ["population prevalence coverage, synthesis bias, competitive member abundance or empirical inclusivity/exclusivity"],
    },
    "tiled-scheme": {
        "items": [
            _item("sequencing_coverage", "Observed sequencing coverage by amplicon", kind="measurement", phase="post-run", source="tiled-amplicon sequencing validation practice", why="Predicted tiling coverage cannot establish read depth or dropout in the actual library/sequencing workflow."),
            _item("pool_balance", "Pool balance and dropout review", kind="measurement", phase="assay-validation", source="tiled-amplicon sequencing validation practice", why="Interaction scores and pool assignment do not measure empirical amplification balance."),
            _item("negative_controls", "Extraction/NTC controls", kind="control", phase="bench-run", source="amplicon sequencing validation practice", why="Controls are required to identify contamination and index/read carry-over that are outside sequence design."),
            _item("variant_refresh", "Primer-mismatch/variant refresh review", kind="decision", phase="design-review", source="PrimalScheme lifecycle practice", why="A released scheme must be re-evaluated against newer variation; a historical design cannot guarantee current coverage.", computable=True),
        ],
        "not_computed": ["read depth, pool balance, dropout rate, library yield, contamination or current epidemiological coverage"],
    },
    "race": {
        "items": [
            _item("rna_integrity", "RNA integrity and input quality", kind="measurement", phase="pre-run", source="RACE validation practice", why="Transcript-end recovery depends on physical RNA integrity that cannot be inferred from a reference sequence."),
            _item("rt_adapter_preparation", "RT/adapter preparation evidence", kind="measurement", phase="pre-run", source="RACE protocol validation", why="The selected 5′/3′ RACE branch assumes a real preparation/adapter state; sequence design cannot prove it."),
            _item("nested_round_identity", "Nested-round product identity, when nested RACE is used", kind="measurement", phase="post-run", source="RACE validation practice", why="A nested design predicts primer geometry but cannot prove the observed transcript-end product.", required=False),
            _item("transcript_end_confirmation", "Transcript-end orthogonal confirmation", kind="measurement", phase="post-run", source="RACE validation practice", why="A candidate end recovered by amplification should be mapped/sequenced; PCRStudio does not infer a true biological transcript end from primer placement."),
        ],
        "not_computed": ["RNA integrity, reverse-transcription yield, cap/poly(A) state, true transcript end or isoform abundance"],
    },
    "sequencing-primer": {
        "items": [
            _item("instrument_chemistry", "Sequencing instrument/chemistry", kind="decision", phase="pre-run", source="Sanger sequencing workflow", why="Primer placement is separable from the facility/instrument chemistry that determines trace performance."),
            _item("trace_quality", "Observed chromatogram/trace quality", kind="measurement", phase="post-run", source="Sanger sequencing workflow", why="Usable read length, mixed signal and basecalling quality are properties of the observed trace, not sequence-design outputs."),
            _item("read_mapping", "Read mapping and expected-direction confirmation", kind="measurement", phase="post-run", source="Sanger sequencing workflow", why="The obtained read must map to the intended template in the expected direction before the design is treated as experimentally successful."),
        ],
        "not_computed": ["chromatogram quality, Phred/basecalling quality, usable read length or facility-specific chemistry performance"],
    },
    "gibson-assembly": {
        "items": [
            _item("assembly_negative_control", "Assembly negative control", kind="control", phase="bench-run", source="Gibson assembly validation practice", why="A negative control distinguishes intended assembly from vector/background recovery."),
            _item("junction_verification", "Assembly-junction sequence verification", kind="measurement", phase="post-run", source="Gibson assembly validation practice", why="Overlap geometry predicts a construct but does not prove the physical junction sequence."),
            _item("construct_identity", "Final construct identity", kind="measurement", phase="post-run", source="Gibson assembly validation practice", why="Independent PCR-product simulation is not full assembly/construct validation; verify the completed construct experimentally."),
        ],
        "not_computed": ["assembly efficiency, fragment stoichiometry effects, colony recovery or complete construct sequence"],
    },
    "site-directed-mutagenesis": {
        "items": [
            _item("parental_template_control", "Parental-template control/removal", kind="control", phase="bench-run", source="site-directed mutagenesis validation practice", why="Primer design cannot establish removal of parental template or background carry-over."),
            _item("clone_screening", "Clone screening", kind="measurement", phase="post-run", source="site-directed mutagenesis validation practice", why="A predicted edited amplicon is not evidence that a recovered clone carries the intended edit."),
            _item("edit_sequence_confirmation", "Edit and surrounding-sequence confirmation", kind="measurement", phase="post-run", source="site-directed mutagenesis validation practice", why="Confirm the intended change and relevant surrounding sequence; PCRStudio does not infer absence of unintended changes from primer design."),
        ],
        "not_computed": ["mutagenesis efficiency, parental-template carry-over, clone frequency or absence of unintended sequence changes"],
    },
    "restriction-cloning": {
        "items": [
            *COMMON,
