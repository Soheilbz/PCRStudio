...
from .primalscheme_adapter import PrimalSchemeImportError, run as run_primalscheme
from .tiling_evidence import TilingEvidenceError, parse_depth_tsv, repair_handoff, scheme_diff, version_transition
...
def _finish(request: dict[str, Any], result: dict[str, Any]) -> dict[str, Any]:
    result["managed_tools"] = {tool: tool_status(tool) for tool in ("primalscheme3", "olivar", "mafft", "primerpooler", "ncbi_blast_plus")}
    evidence=None
    if request.get("tilingDepthTsv") not in (None, ""):
        try:
            evidence=parse_depth_tsv(str(request.get("tilingDepthTsv")), threshold=float(request.get("tilingDropoutThreshold") if request.get("tilingDropoutThreshold") is not None else 20.0))
        ...
        result["dropout_repair_handoff"]=repair_handoff(evidence, existing_bed=str(request.get("existingBed") or ((result.get("scheme_artifacts") or {}).get("primer_bed") or {}).get("content") or ""))
    old_bed=str(request.get("existingBed") or "")
    new_bed=str(((result.get("scheme_artifacts") or {}).get("primer_bed") or {}).get("content") or "")
    if old_bed and new_bed:
        diff=scheme_diff(old_bed,new_bed)
        result["scheme_version_diff"]=diff
        result["version_transition"]=version_transition(str(request.get("schemeVersion") or "v0.0.0"),diff)
    return result
