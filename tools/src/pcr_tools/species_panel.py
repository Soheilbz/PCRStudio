...
def parse_accession_version_manifest(text: str) -> tuple[str, ...]:
    ...
    for lineno, raw in enumerate(str(text).splitlines(), start=1):
        ...
        if not _ACCESSION_VERSION.fullmatch(token):
            raise SpeciesPanelError(f"line {lineno} must begin with a versioned accession such as NC_000001.11; got {token!r}")
        if token in seen:
...
def surveillance_diff(old: tuple[str, ...] | list[str], new: tuple[str, ...] | list[str]) -> SurveillanceDiff:
    def base(accession: str) -> str:
        return accession.rsplit(".", 1)[0]
    old_map = {base(x): x for x in old}
    new_map = {base(x): x for x in new}
    changed = tuple(sorted((old_map[b], new_map[b]) for b in old_map.keys() & new_map.keys() if old_map[b] != new_map[b]))
    unchanged = tuple(sorted(old_map[b] for b in old_map.keys() & new_map.keys() if old_map[b] == new_map[b]))
    ...
    return SurveillanceDiff(added=added, removed=removed, version_changed=changed, unchanged=unchanged)
...
def parse_record_metadata_manifest(text: str) -> tuple[SpeciesRecordMetadata, ...]:
    ...
    for lineno, raw in enumerate(str(text).splitlines(), start=1):
        ...
        if not record_id or any(ch.isspace() for ch in record_id):
            raise SpeciesPanelError(f"line {lineno} record_id must be a non-empty FASTA identifier without whitespace")
        ...
        if len(parts) >= 5 and parts[4].strip():
            ...
            if not (weight > 0.0) or weight == float('inf'):
                raise SpeciesPanelError(f"line {lineno} weight must be finite and greater than zero")
...
def record_metadata_sha256(rows: tuple[SpeciesRecordMetadata, ...] | list[SpeciesRecordMetadata]) -> str:
    canonical = '\n'.join(
        '\t'.join([
            row.record_id,
        ...
        for row in sorted(rows, key=lambda item: (item.record_id, item.accession_version, item.role, item.topology, item.group or '', -1.0 if item.weight is None else item.weight))
    ) + '\n'
    return hashlib.sha256(canonical.encode('utf-8')).hexdigest()
...
def validate_record_metadata(
    ...
    *,
    ...
    if set(by_record) != expected_ids:
        ...
        raise SpeciesPanelError(f"record-metadata manifest must cover every FASTA record exactly once; missing={missing}, extra={extra}")
    if {row.accession_version for row in rows} != set(accessions):
        ...
        raise SpeciesPanelError(f"record-metadata accessions must exactly match accession manifest; missing={missing}, extra={extra}")
    ...
    for row in rows:
        ...
        if row.role != expected_role:
            raise SpeciesPanelError(f"record {row.record_id!r} is {expected_role} FASTA but metadata role is {row.role}")
    ...
    normalized_weights = (
        {row.record_id: float(row.weight) / total_weight for row in weighted if row.weight is not None}
...
def parse_accession_authority_manifest(text: str) -> tuple[AccessionAuthorityRecord, ...]:
    ...
    for lineno, raw in enumerate(str(text).splitlines(), start=1):
        ...
        if accession in seen:
            raise SpeciesPanelError(f"duplicate accession.version in accession authority manifest: {accession}")
        try:
        ...
        if status == 'replaced':
            if replacement is None or not _ACCESSION_VERSION.fullmatch(replacement):
                raise SpeciesPanelError(f"line {lineno} replaced accession requires a versioned replacement accession")
            if replacement == accession:
                raise SpeciesPanelError(f"line {lineno} replacement accession must differ from the superseded accession")
        elif replacement is not None:
            raise SpeciesPanelError(f"line {lineno} status {status} must not declare a replacement accession")
        ...
        rows.append(AccessionAuthorityRecord(accession, taxid, status, replacement, sequence_snapshot, taxonomy_snapshot))
        seen.add(accession)
    if not rows:
...
def accession_authority_sha256(rows: tuple[AccessionAuthorityRecord, ...] | list[AccessionAuthorityRecord]) -> str:
    canonical = '\n'.join(
        '\t'.join([
            row.accession_version, str(row.taxid), row.status,
            row.replacement_accession_version or '', row.sequence_database_snapshot or '', row.taxonomy_snapshot or '',
        ])
...
def validate_accession_authority(
    ...
    *,
    ...
    replaced = {row.accession_version: row.replacement_accession_version for row in rows if row.status == 'replaced'}
    ...
    if suppressed:
        raise SpeciesPanelError("species panel contains suppressed accession(s): " + ", ".join(suppressed))
    if replaced:
        detail = ", ".join(f"{old}->{new}" for old, new in sorted(replaced.items()))
        raise SpeciesPanelError("species panel contains replaced accession(s); update/review replacements before design: " + detail)
    sequence_mismatch = sorted(
        row.accession_version for row in rows
        if row.sequence_database_snapshot and row.sequence_database_snapshot != sequence_database_snapshot
    )
    ...
    if sequence_mismatch:
        raise SpeciesPanelError("accession authority sequence-database snapshot disagrees with the declared panel snapshot for: " + ", ".join(sequence_mismatch))
    if taxonomy_mismatch:
        raise SpeciesPanelError("accession authority taxonomy snapshot disagrees with the declared panel snapshot for: " + ", ".join(taxonomy_mismatch))
    taxids = sorted({row.taxid for row in rows})
    return {
        'record_count': len(rows),
        'status_counts': {status: sum(row.status == status for row in rows) for status in ('current','suppressed','replaced')},
