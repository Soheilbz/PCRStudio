...
from .degenerate import EXPANSION, expand, matches, reverse_complement
...
def parse_panel_metadata(raw: Any, names: Iterable[str]) -> dict[str, dict[str, Any]]:
    ...
    if extra:
        raise ValueError("panel_metadata contains record(s) absent from the alignment: " + ", ".join(extra))
    ...
    for name, value in records.items():
        ...
        if "stratum" in item and (not isinstance(item["stratum"], str) or not item["stratum"].strip()):
...
def panel_qc(records: list[tuple[str, str]], metadata: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    for name, seq in normalized:
        ...
        per_record.append({
            "name":name,"length":len(seq),"gap_bases":gaps,"gap_fraction":round(gaps/max(1,len(seq)),6),
            "ambiguous_bases":ambiguities,"invalid_symbols":invalid,
    ...
    return {
        "records":per_record,
        "duplicate_sequence_groups":[{"records":sorted(names),"count":len(names)} for names in sorted(duplicate_groups.values(),key=lambda x:tuple(x))],
        "length_min":min(lengths) if lengths else 0,"length_max":max(lengths) if lengths else 0,
...
def formulation_for_pair(pair: dict[str, Any], mode: str, total_nm: float | None) -> dict[str, Any]:
    ...
    for role in ("left","right"):
        site=pair[role]; deg=int(site["degeneracy"]); members=_member_list(site["sequence"])
        primers[role]={
            "iupac":site["sequence"],"degeneracy":deg,
            "concrete_members":members if mode in {"defined-oligo-pool","discrete-subprimer-mixture"} else None,
            "exact_members_enumerated":members is not None,
            "declared_total_concentration_nM":total_nm,
            "nominal_equal_member_concentration_nM":round(total_nm/deg,6) if total_nm is not None else None,
        }
    return {
        "mode":mode,"primers":primers,
...
def scan_nontarget_pair(pair: dict[str, Any], records: list[tuple[str,str]], product_min: int, product_max: int) -> dict[str, Any]:
    """Exact IUPAC-compatible product scan over a finite supplied non-target panel."""
    left=pair["left"]["sequence"].upper(); right=pair["right"]["sequence"].upper()
    reverse_binding=reverse_complement(right)
    hits=[]
    for name,raw in records:
        seq="".join(c for c in raw.upper() if not c.isspace()).replace("U","T").replace("-","")
        if not seq: continue
        left_sites=_scan_one_primer(left,seq); right_sites=_scan_one_primer(reverse_binding,seq)
        products=[]
        for l in left_sites:
            for r in right_sites:
                size=r+len(right)-l
                if r>=l+len(left) and product_min <= size <= product_max:
                    products.append({"start":l,"end":r+len(right),"size":size})
        if products:
            hits.append({"record":name,"products":products[:20],"product_count":len(products)})
    return {
        "records_tested":len(records),"records_with_exact_compatible_product":len(hits),"hits":hits,
...
def coverage_evidence(pair: dict[str, Any], names: list[str], rows: list[str], metadata: dict[str, dict[str, Any]]) -> dict[str, Any]:
    """Per-record, weighted and stratified exact coverage for one pair."""
    left=pair["left"]; right=pair["right"]
    l0=int(left["start"])-1; r0=int(right["start"])-1
    covered=[]; total_weight=0.0; covered_weight=0.0; strata=defaultdict(lambda:{"covered":0,"total":0,"covered_weight":0.0,"total_weight":0.0})
    for name,row in zip(names,rows,strict=True):
        weight=float(metadata.get(name,{}).get("weight",1.0)); stratum=str(metadata.get(name,{}).get("stratum") or "unstratified")
        total_weight+=weight; strata[stratum]["total"]+=1; strata[stratum]["total_weight"]+=weight
        ok=matches(left["sequence"],row[l0:l0+int(left["length"])]) and matches(reverse_complement(right["sequence"]),row[r0:r0+int(right["length"])])
        if ok:
            covered.append(name); covered_weight+=weight; strata[stratum]["covered"]+=1; strata[stratum]["covered_weight"]+=weight
    return {
        "covered_records":covered,"covered_count":len(covered),"total_records":len(names),
        "weighted_coverage":round(covered_weight/total_weight,6) if total_weight else None,
        "strata":{key:{**value,"coverage":round(value["covered"]/value["total"],6) if value["total"] else None,"weighted_coverage":round(value["covered_weight"]/value["total_weight"],6) if value["total_weight"] else None} for key,value in sorted(strata.items())},
        "population_inference":"not-established",
    }


def alternative_alignment_evidence(pair: dict[str,Any], alternative_records: list[tuple[str,str]]) -> dict[str,Any]:
    ...
    """
    names=[n for n,_ in alternative_records]; rows=["".join(c for c in s.upper() if not c.isspace()).replace("U","T") for _,s in alternative_records]
    if not rows or len({len(r) for r in rows})!=1:
        return {"status":"unresolved","reason":"alternative alignment rows do not share one width"}
    l0=int(pair["left"]["start"])-1; r0=int(pair["right"]["start"])-1
    max_end=max(l0+int(pair["left"]["length"]),r0+int(pair["right"]["length"]))
    if max_end>len(rows[0]): return {"status":"unresolved","reason":"candidate coordinates exceed alternative alignment width"}
    covered=[]
    for name,row in zip(names,rows,strict=True):
        ok=matches(pair["left"]["sequence"],row[l0:l0+int(pair["left"]["length"])]) and matches(reverse_complement(pair["right"]["sequence"]),row[r0:r0+int(pair["right"]["length"])])
        if ok: covered.append(name)
    fraction=len(covered)/len(rows)
    return {"status":"stable-at-reported-columns" if fraction==1.0 else "alignment-sensitive","covered":len(covered),"of":len(rows),"coverage":round(fraction,6),"claim_boundary":"Column-coordinate sensitivity evidence only; PCRStudio does not claim that two aligners define identical biological homology."}
...
def amplicon_informativeness(
    ...
    if not rows or start < 0 or end <= start or end > min(len(row) for row in rows):
        return {"status": "unresolved", "reason": "reported primer coordinates do not define a common alignment interval"}
    ...
    amplicons = [row[start:end].replace("-", "").replace(".", "").upper().replace("U", "T") for row in rows]
    ...
    return {
        ...
        "mean_shannon_entropy_bits": round(sum(entropies) / len(entropies), 6) if entropies else 0.0,
