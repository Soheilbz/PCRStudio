...
def _specific_candidates(
    sequence: str, competing: str, end: int, limits: Constraints, conditions: dict[str, float]
) -> list[dict[str, Any]]:
    found = []
    for length in range(limits.length_min, limits.length_max + 1):
        ...
        if (
            not limits.tm_min <= measured.tm <= limits.tm_max
            or not limits.gc_min <= measured.gc_percent <= limits.gc_max
        ):
            continue
    ...
    found.sort(
        key=lambda x: (not x["three_prime_unique_10mer"], abs(x["tm"] - limits.tm_opt), x["length"])
    )
    return found
...
def _common_reverse(
    ref_seq: str,
    alt_seq: str,
    junction_ref_end: int,
    limits: Constraints,
    conditions: dict[str, float],
    left_starts: tuple[int, int],
) -> dict[str, Any] | None:
    # Common reverse must be an exact sequence in the shared downstream flank.
    downstream_start = max(junction_ref_end, 0)
    for right_end in range(
        min(len(ref_seq), downstream_start + limits.product_min),
        min(len(ref_seq), downstream_start + limits.product_max + 60),
    ):
        for length in range(limits.length_min, limits.length_max + 1):
            ...
            if all(
                limits.product_min <= size <= limits.product_max
                for size in (ref_product, alt_product)
            ):
                return {
                    "sequence": oligo,
                    "at_ref": ref_match,
                    ...
                    "tm": measured.tm,
                    "gc_percent": measured.gc_percent,
                    "product_sizes": {"REF": ref_product, "ALT": alt_product},
                }
    return None


def design_plus_minus(
    template: str, variant: NormalizedVariant, limits: Constraints, conditions: dict[str, float]
) -> dict[str, Any]:
    ...
    if not ref_candidates or not alt_candidates:
        return {
            "orderable": False,
            "why": "No pair of allele-junction primers survived thermodynamic and competing-allele exact-match screening.",
            "primers": [],
            "partners": [],
        }
    ...
    for r in ref_candidates[:12]:
        for a in alt_candidates[:12]:
            common = _common_reverse(
                ref_seq,
                alt_seq,
                variant.at + len(variant.ref),
                limits,
                conditions,
                (r["at"], a["at"]),
            )
            if common is None:
                continue
            score = (
                abs(r["tm"] - a["tm"]) + abs(r["tm"] - common["tm"]) + abs(a["tm"] - common["tm"])
            )
            if best is None or score < best[0]:
                best = (score, r, a, common)
    if best is None:
        return {
            "orderable": False,
            "why": "Allele-junction primers were found, but no shared downstream common reverse primer satisfied the declared product/Tm window on both alleles.",
            "primers": [],
            "partners": [],
        }
    _, r, a, common = best
    ...
    for index, (label, p) in enumerate((("REF", r), ("ALT", a))):
        primers.append(
            {
                "name": f"{label}-specific",
                "allele": label,
                "strand": "plus",
                "sequence": p["sequence"],
                "ordered_sequence": kasp.attach(p["sequence"], index),
                "at": p["at"],
                "length": p["length"],
                "tm": p["tm"],
                "gc_percent": p["gc_percent"],
                "cassette": kasp.describe(index, p["sequence"], p["tm"]),
    ...
    dimer_values = [
        pair_dimer(common["sequence"], specific["sequence"], **conditions).dg for specific in (r, a)
    ]
    partner = {
        "for_alleles": ["REF", "ALT"],
        ...
        "strand": "minus",
        "tm": common["tm"],
        "gc_percent": common["gc_percent"],
        "length": common["length"],
        "product_size": max(common["product_sizes"].values()),
        "product_sizes": common["product_sizes"],
        "cross_dimer_dg": min(dimer_values),
        "cross_dimer_dg_basis": "worst (most negative) exact thermodynamic result across common-vs-REF and common-vs-ALT primer pairs",
        "tm_difference": max(abs(common["tm"] - r["tm"]), abs(common["tm"] - a["tm"])),
        "note": "One exact shared downstream common primer used on both reconstructed allele sequences.",
    }
    return {
        "orderable": True,
        "why": "",
        ...
        "partners": [partner],
        "junction_model": {
            "ref_junction": ref_end,
            "alt_junction": alt_end,
            "ref_length": len(ref_seq),
            "alt_length": len(alt_seq),
            "model": "explicit reconstructed REF/ALT junctions",
        ...
        "call_model": {
            "states": [
                "presence",
                "absence",
                "mixed/heterozygous-when-biologically-applicable",
                "no-template",
                "no-call",
                "weak",
                "ambiguous-cluster",
            ...
            "claim_boundary": "A predicted design is not a genotype/presence call until endpoint controls and cluster evidence are reviewed.",
        },
    }
