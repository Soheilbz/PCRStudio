...
def optical_authority(value: Any, *, panel: Any = None, reporter: str | None = None) -> dict[str, Any]:
    ...
    if payload.get("schema") != "pcrstudio.qpcr-optical-profile.v1":
        raise ProbeClosureError("probe optical authority requires schema pcrstudio.qpcr-optical-profile.v1")
    authority_id = str(payload.get("authority_id") or "").strip()
    instrument = str(payload.get("instrument") or "").strip()
    version = str(payload.get("version") or "").strip()
    channels = payload.get("channels")
    if not authority_id or not instrument or not version or not isinstance(channels, list) or not channels:
        raise ProbeClosureError("optical authority requires authority_id, instrument, version and non-empty channels")
    ...
    if reporter:
        ...
        validations.append({"target":"current-assay","reporter":reporter,"compatible_channels":compatible,"valid":bool(compatible)})
        if not compatible:
            raise ProbeClosureError(f"reporter `{reporter}` is not present in the supplied optical authority")
    ...
    for raw in panel or []:
        ...
        validations.append({"target":target,"channel":channel or None,"reporter":rep or None,"valid":valid})
        if channel:
            if channel in seen_channels:
                raise ProbeClosureError(f"multiplex optical channel `{channel}` is assigned more than once")
            seen_channels.add(channel)
        if channel and rep and not valid:
            raise ProbeClosureError(f"multiplex target `{target}` maps reporter `{rep}` to unsupported channel `{channel}`")
    return {
        "status":"validated-authority",
        "schema":payload["schema"],"authority_id":authority_id,"instrument":instrument,"version":version,
        "validations":validations,"decision_impact":"validation-only",
...
def multiplex_interactions(panel: Any, current: dict[str, str], *, mv_conc: float = 50.0, dv_conc: float = 1.5, dntp_conc: float = 0.6, dna_conc: float = 50.0) -> dict[str, Any] | None:
    if not panel:
        return None
    if not isinstance(panel, list):
        raise ProbeClosureError("probe_multiplex_panel must be an array")
    oligos: list[tuple[str,str]] = [(f"current:{role}", seq) for role,seq in current.items() if seq]
    for idx,row in enumerate(panel):
        ...
        for role,seq in values.items():
            ...
            if cleaned: oligos.append((f"{label}:{role}",cleaned))
    if len(oligos) <= len(current):
        return {"status":"unresolved-peer-oligo-sequences-not-supplied","pairs":[],"decision_impact":"diagnostic-only"}
    pairs=[]
    for i,(an,a) in enumerate(oligos):
        for bn,b in oligos[i+1:]:
            if an.split(':',1)[0] == bn.split(':',1)[0]:
                continue
            try:
                dg=float(primer3.calcHeterodimer(a,b,mv_conc=mv_conc,dv_conc=dv_conc,dntp_conc=dntp_conc,dna_conc=dna_conc).dg)/1000.0
            except Exception as exc:
                pairs.append({"a":an,"b":bn,"status":"calculation-error","detail":str(exc)[:240]})
                continue
            pairs.append({"a":an,"b":bn,"heterodimer_dg_kcal_mol":round(dg,3)})
    pairs.sort(key=lambda row: row.get("heterodimer_dg_kcal_mol",999.0))
    return {"status":"calculated","pairs":pairs,"decision_impact":"diagnostic-only","threshold":"none-universal","note":"All-vs-all peer interaction evidence is reported without inventing a universal rejection threshold."}
...
def combined_signal(off_targets: dict[str, Any], probe: str, contigs: list[Any]) -> dict[str, Any]:
    ...
    if not isinstance(products,list):
        return {"status":"unresolved","compatible_product_count":None,"decision_impact":"validation-only"}
    by_name={str(c.name):str(c.sequence).upper() for c in contigs}
    p=probe.upper(); rc=reverse_complement(p)
    compatible=[]
    for row in products:
        if not isinstance(row,dict): continue
        seq=by_name.get(str(row.get("contig") or ""),"")
        try: start=max(0,int(row.get("start"))-1); end=int(row.get("end"))
        except Exception: continue
        product=seq[start:end]
        if p in product or rc in product:
            compatible.append({"contig":row.get("contig"),"start":row.get("start"),"end":row.get("end"),"size":row.get("size")})
    return {"status":"computed","compatible_product_count":len(compatible),"compatible_products":compatible,"decision_impact":"validation-only","note":"A compatible signal requires an amplifiable product and a probe site on that product; this is not a fluorescence probability model."}
