...
def _canonical(cands: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [
        {
            key: item[key]
            for key in ("candidate_id", "sequence", "template_start", "length", "strand")
        }
        for item in cands
    ]
...
def candidate_set_sha256(cands: list[dict[str, Any]]) -> str:
    return sha256(
        json.dumps(_canonical(cands), sort_keys=True, separators=(",", ":")).encode()
    ).hexdigest()


def candidates(
    template: str, *, start: int, end: int, minimum: int = 13, maximum: int = 25
) -> list[dict[str, Any]]:
    ...
    for strand in ("forward", "reverse"):
        ...
        interval_start, interval_end = (
            (start, end) if strand == "forward" else (len(seq) - end, len(seq) - start)
        )
    ...
    found.sort(
        key=lambda item: (
            abs((item["template_start"] + item["length"] / 2) - midpoint),
            item["length"],
            item["candidate_id"],
        )
    )
...
def exchange_manifest(
    cands: list[dict[str, Any]], *, authority_id: str, mode: str = "export-candidates"
) -> dict[str, Any]:
...
def apply_import(cands: list[dict[str, Any]], payload: Any, *, authority_id: str) -> dict[str, Any]:
    if isinstance(payload, str):
        try:
            ...
            raise MgbAuthorityError(
                f"probe_mgb_authority_payload is not valid JSON: {exc}"
            ) from exc
    if (
        not isinstance(payload, dict)
        or payload.get("schema") != "pcrstudio.mgb-authority-result.v1"
    ):
    ...
    if str(payload.get("authority_id") or "") != authority_id:
        raise MgbAuthorityError(
            "MGB authority_id does not match the selected reviewed MGB authority"
        )
    expected_hash = candidate_set_sha256(cands)
    supplied_hash = str(payload.get("candidate_set_sha256") or "").strip()
    if supplied_hash != expected_hash:
        raise MgbAuthorityError(
            "MGB candidate_set_sha256 does not match this exported candidate set"
        )
    ...
    if (
        ...
        or not 30 <= float(target_raw) <= 100
    ):
    ...
    for row in rows:
        ...
        if cid in seen or cid not in by_id:
            raise MgbAuthorityError(
                f"MGB imported candidate_id `{cid}` is duplicate or not in the exported candidate set"
            )
        seen.add(cid)
        tm = row.get("tm_c")
