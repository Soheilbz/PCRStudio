...
_REGISTRY = json.loads(
    files("pcr_tools").joinpath(*_RESOURCE.split("/")).read_text(encoding="utf-8")
)
...
def active_for_run(
    module_id: str | None, command: str, request: dict[str, Any], result: dict[str, Any]
) -> list[dict[str, Any]]:
    roles = _roles_for_module(module_id)
    ...
    if module_id == "site-directed-mutagenesis":
        topology = str(
            result.get("topology")
            or request.get("mutagenesis_topology")
            or request.get("topology")
            or ""
        )
        protocol = str(
            (result.get("protocol") or {}).get("selection")
            or request.get("post_amplification_protocol")
            or ""
        )
...
def context_references(context_id: str) -> list[dict[str, Any]]:
    ...
    if not isinstance(context, dict):
        return []
    return [
        _row(str(method_id), use_role="reference") for method_id in context.get("references", [])
    ]
...
def enforce_scientific_strict(rows: list[dict[str, Any]], *, context: str) -> None:
    ...
    blocked = [
        row
        for row in rows
        if row.get("use_role", "").startswith("active")
