...
def resolve_cloning_coding_context(request: dict[str, Any], *, insert_sequence: str) -> dict[str, Any]:
    ...
    stop_policy = str(request.get("cloning_stop_codon_policy") or ("not-applicable" if intent == "noncoding" else "preserve"))
    ...
    if linker_aa and any(not (c.isalpha() or c in "*-") for c in linker_aa):
        raise ValueError("cloning_linker_aa must contain amino-acid letters only, with optional * or - notation")

    if intent == "noncoding":
        if any(request.get(key) is not None for key in ("cloning_cds_start", "cloning_cds_end", "cloning_vector_junction_frame")):
            raise ValueError("noncoding restriction cloning cannot carry CDS/frame coordinates")
        if stop_policy != "not-applicable":
            raise ValueError("noncoding restriction cloning requires cloning_stop_codon_policy=not-applicable")
        return {
            "intent": intent,
    ...
    if isinstance(start, bool) or not isinstance(start, int) or isinstance(end, bool) or not isinstance(end, int):
        raise ValueError("coding restriction cloning requires integer cloning_cds_start/cloning_cds_end")
    if not (0 <= start < end <= len(insert_sequence)):
        raise ValueError(f"cloning CDS interval must be within final insert 0..{len(insert_sequence)}")
    ...
    if stop_policy == "remove" and terminal_is_stop:
        raise ValueError("cloning_stop_codon_policy=remove requires a final insert/CDS that does not include the terminal stop codon")
    if stop_policy == "not-applicable":
