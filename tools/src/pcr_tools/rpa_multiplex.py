...
def resolve(panel: Any, *, enabled: bool) -> dict[str, Any] | None:
    ...
    if not isinstance(panel, list) or not panel:
        if strict():
            raise ValueError(
                "Scientific-Strict RPA multiplex requires selected peer assays plus empirical evidence references; sequence-only multiplex is not validated RPA"
            )
        panel = []
    if len(panel) >= RPA_MULTIPLEX_SOFTWARE_MAX_TARGETS:
        raise ValueError(
            "RPA multiplex planning accepts at most four peer assays (five total including the current target); this is a planning bound, not wet-lab qualification"
        )
    normalized = []
    seen = set()
    missing = []
    for row in panel:
        if not isinstance(row, dict):
            raise ValueError("each rpa_multiplex_panel row must be an object")
        target = str(row.get("target") or "").strip()
        f = "".join(
            str(row.get("forward_primer") or row.get("forwardPrimer") or "").split()
        ).upper()
        r = "".join(
            str(row.get("reverse_primer") or row.get("reversePrimer") or "").split()
        ).upper()
        if not target or target in seen:
            raise ValueError("RPA multiplex target identities must be non-empty and unique")
        seen.add(target)
        if not f or not r or any(base not in "ACGT" for base in f + r):
            raise ValueError(
                f"RPA peer {target} requires unambiguous selected forward/reverse primers"
            )
        evidence = str(
            row.get("empirical_evidence_ref") or row.get("empiricalEvidenceRef") or ""
        ).strip()
        if not evidence:
        ...
        normalized.append(
            {
                ...
                "detection_identity": str(
                    row.get("detection_identity") or row.get("detectionIdentity") or ""
                ).strip()
                or None,
                "empirical_evidence_ref": evidence or None,
            }
        )
    if strict() and missing:
        raise ValueError(
            "Scientific-Strict RPA multiplex requires empirical evidence reference for every selected peer assay; missing: "
            + ", ".join(missing)
        )
    ...
    return {
        "status": "empirical-panel-context-complete"
        if normalized and not missing
        else "development-panel-context-incomplete",
        "panel": normalized,
        "panel_sha256": digest,
        "target_count": 1 + len(normalized),
        "software_planning_bound": RPA_MULTIPLEX_SOFTWARE_MAX_TARGETS,
...
def extend_order_sheet(order_sheet: list[dict[str, Any]], context: dict[str, Any] | None) -> None:
    ...
    for peer in context.get("panel") or []:
        ...
        for suffix, sequence in (
            ("F", peer.get("forward_primer")),
            ("R", peer.get("reverse_primer")),
        ):
            ...
            order_sheet.append(
                {
                    ...
                    "length": len(seq),
                    "gc_percent": round(
                        100.0 * sum(base in {"G", "C"} for base in seq) / max(len(seq), 1), 1
                    ),
