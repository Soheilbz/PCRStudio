...
def test_current_protocol_catalogue_and_substrate_partition():
    ...
    assert len(reg)==72
    dna=[k for k,v in reg.items() if v.get('supports_dna',True) and not v.get('supports_rna',False)]
    rna=[k for k,v in reg.items() if v.get('supports_rna',False) and not v.get('supports_dna',True)]
    dual=[k for k,v in reg.items() if v.get('supports_dna',True) and v.get('supports_rna',False)]
...
def test_new_exact_product_authorities_exist_without_sequence_impact():
    for pid in ['jena-pcr387','nzy-md0696','nippon-dr0401','yeasen-16730','vazyme-rp711','agdia-lmx54700']:
