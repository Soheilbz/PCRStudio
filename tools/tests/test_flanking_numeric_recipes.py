...
def test_powertrack_requires_source_cycling_branch_and_resolves_20_ul() -> None:
    ...
    assert any(
        row["id"] == "powertrack-cycling-profile"
        for row in unresolved["unresolved_numeric_dependencies"]
    )
    recipe = resolve_numeric_recipe(
...
def test_kod_long_length_drives_only_documented_extension_regime() -> None:
    short = resolve_numeric_recipe(
        "toyobo-kod-long-kml101", "long-range-pcr", scenario={"target_length_kb": 8.0}
    )
    long = resolve_numeric_recipe(
        "toyobo-kod-long-kml101", "long-range-pcr", scenario={"target_length_kb": 12.0}
    )
    ...
    with pytest.raises(ValueError):
        resolve_numeric_recipe(
            "toyobo-kod-long-kml101", "long-range-pcr", scenario={"target_length_kb": 51.0}
        )
...
def test_qiacuity_refuses_to_invent_nanoplate_reaction_volume() -> None:
    ...
    assert any(
        row["id"] == "qiacuity-nanoplate-format"
        for row in unresolved["unresolved_numeric_dependencies"]
    )
...
def test_colony_vendor_protocols_keep_distinct_source_conditioned_lysis_times() -> None:
    m0482 = resolve_numeric_recipe(
        "neb-onetaq-m0482-colony", "colony-pcr", scenario={"preparation": "direct-transfer"}
    )
