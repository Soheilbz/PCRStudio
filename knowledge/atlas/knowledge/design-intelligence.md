...
| `primer3-py` | `OPTIONAL` development/test adapter | current `2.3.0`; Python `>=3.8`; never silently substitute its bundled Primer3 behavior for the pinned production executable | GPLv2 Python/Cython wrapper; import/linking has a different licensing boundary from process execution and therefore requires deliberate product/legal review ([primer3-py 2.3.0](https://pypi.org/project/primer3-py/2.3.0/)) |
